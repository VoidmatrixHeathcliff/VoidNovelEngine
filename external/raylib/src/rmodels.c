...
Model LoadModel(const char *fileName)
{
    ...
    model.transform = RMATH_MatrixIdentity();
...
void DrawMesh(Mesh mesh, Material material, Matrix transform)
{
    ...
    Matrix matModel = RMATH_MatrixIdentity();
    Matrix matView = rlGetMatrixModelview();
    Matrix matModelView = RMATH_MatrixIdentity();
    ...
    matModel = RMATH_MatrixMultiply(transform, rlGetMatrixTransform());
    ...
    matModelView = RMATH_MatrixMultiply(matModel, matView);
    ...
    for (int eye = 0; eye < eyeCount; eye++)
    {
        ...
        Matrix matModelViewProjection = RMATH_MatrixIdentity();
        if (eyeCount == 1) matModelViewProjection = RMATH_MatrixMultiply(matModelView, matProjection);
        else
        {
            // Setup current eye viewport (half screen width)
            rlViewport(eye*rlGetFramebufferWidth()/2, 0, rlGetFramebufferWidth()/2, rlGetFramebufferHeight());
            matModelViewProjection = RMATH_MatrixMultiply(RMATH_MatrixMultiply(matModelView, rlGetMatrixViewOffsetStereo(eye)), rlGetMatrixProjectionStereo(eye));
        }
...
void DrawMeshInstanced(Mesh mesh, Material material, const Matrix *transforms, int instances)
{
    ...
    matModelView = RMATH_MatrixMultiply(rlGetMatrixTransform(), matView);
        ...
        for (int i = 0; i < model.meshCount; i++)
        {
            if (model.meshes[i].boneMatrices)
            {
                ...
                for (int boneId = 0; boneId < model.meshes[i].boneCount; boneId++)
                {
                    ...
                    Matrix boneMatrix = RMATH_MatrixMultiply(RMATH_MatrixMultiply(
...
void DrawModelEx(Model model, Vector3 position, Vector3 rotationAxis, float rotationAngle, Vector3 scale, Color tint)
{
    ...
    Matrix matTransform = RMATH_MatrixMultiply(RMATH_MatrixMultiply(matScale, matRotation), matTranslation);

    // Combine model transformation matrix (model.transform) with matrix generated by function parameters (matTransform)
    model.transform = RMATH_MatrixMultiply(model.transform, matTransform);
...
static Model LoadIQM(const char *fileName)
{
    ...
    */
    ...
    for (int i = 0; i < model.meshCount; i++)
    {
        ...
        {
            model.meshes[i].boneMatrices[j] = RMATH_MatrixIdentity();
        }
    }
    ...
    if (result == cgltf_result_success)
    {
        ...
        for (unsigned int i = 0; i < data->nodes_count; i++)
        {
            ...
            for (unsigned int p = 0; p < mesh->primitives_count; p++)
            {
                ...
                {
                    model.meshes[meshIndex].boneMatrices[j] = RMATH_MatrixIdentity();
                }
