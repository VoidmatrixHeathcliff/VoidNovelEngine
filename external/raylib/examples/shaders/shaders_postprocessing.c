...
int main(void)
{
    ...
    shaders[FX_CRT] = LoadShader(0, TextFormat("resources/shaders/glsl%i/crt.fs", GLSL_VERSION));
    ...
    RenderTexture2D target_2 = LoadRenderTexture(screenWidth, screenHeight);
    ...
    while (!WindowShouldClose())        // Detect window close button or ESC key
    {
        ...
        else if (IsKeyPressed(KEY_UP)) intensity += 0.05f;
        else if (IsKeyPressed(KEY_DOWN)) intensity -= 0.05f;
        ...
        if (currentShader == FX_CRT)
        {
            float resolution[2] = { (float)screenWidth, (float)screenHeight };
            SetShaderValue(shaders[currentShader], GetShaderLocation(shaders[currentShader], "iResolution"), resolution, SHADER_UNIFORM_VEC2);

            float time = (float)GetTime();
            SetShaderValue(shaders[currentShader], GetShaderLocation(shaders[currentShader], "TIME"), &time, SHADER_UNIFORM_FLOAT);

            SetShaderValue(shaders[currentShader], GetShaderLocation(shaders[currentShader], "crtStrength"), &intensity, SHADER_UNIFORM_FLOAT);
        }
        ...
        BeginDrawing();
            ...
            //        BeginShaderMode(shaders[FX_BLOOM]);
            //            // NOTE: Render texture must be y-flipped due to default OpenGL coordinates (left-bottom)
            //            DrawTextureRec(target.texture, (Rectangle) { 0, 0, (float)target.texture.width, (float)-target.texture.height }, (Vector2) { 0, 0 }, WHITE);
            //        EndShaderMode();
            //    EndTextureMode();

            //    BeginShaderMode(shaders[FX_CRT]);
            //        // NOTE: Render texture must be y-flipped due to default OpenGL coordinates (left-bottom)
            //        DrawTextureRec(target_2.texture, (Rectangle) { 0, 0, (float)target.texture.width, (float)-target.texture.height }, (Vector2) { 0, 0 }, WHITE);
            //    EndShaderMode();
            //}
            //else
            {
