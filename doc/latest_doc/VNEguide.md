...
​	**②** 为 **项目设置** 窗口：用于调整 **预览画面尺寸**、**最终发布画面尺寸** 以及 **预览画面的渲染采样模式**。

​	**③** 为 **发布设置** 窗口：用于设置 **作品打包** 的相关参数。

​	**④** 为 **资产视图** 窗口：用于存放 **人物、背景、音频等资产**，方便查找资源。

​	**⑤** 为 **流程视图** 窗口：用于编辑游戏进程的流程图逻辑窗口，用简单的 **拖拽** 和 **连接** 即可完成游戏逻辑。

​	**⑥** 为 **控制台** 窗口：用于输出使用引擎时所产生的 **日志** 信息或 **报错** 信息等，方便调试。

​	**⑦** 为 **监控视图** 窗口：针对深度开发者使用，用于显示当前流程图中所有元素的信息。

​	**⑧** 为 **样式设计视图** 窗口：**该功能仍在开发中**。
...
在 **VoidNovelEngine.exe** 文件同级目录下的 **application** 文件夹内的 **resources** 文件夹内，将我们 **人物素材**、**视频素材**、 **背景素材**、**音效或音乐素材** 导入。

​	**支持的图片格式**：**PNG、JPG、JPEG、TIF、TIFF、WebP、AVIF**。

​	**支持的视频格式**：**MP4、AVI、MKV、FLV、MOV、WebM**。

​	**支持的音效格式**：**WAV、MP3、OGG、FLAC**。

​	**支持的字体格式**：**TTF、OTF**。

​	**着色器支持说明**：引擎支持加载以 **.glsl** 或 **.fs** 为后缀的 **GLSL 着色器文件**

> [!NOTE]
>
> **注**：在 **resources** 文件夹内部如需对文件进行 **增删** 等操作，需要重启引擎来 **刷新** 资产信息，如遇到无法导入的问题，请先参考支持素材格式。
...
导入成功后在引擎主界面左下方 **资产视图** 窗口中可以找到我们刚刚导入资源的 **名称** 以及对应类型的 **图标**。
...
当该节点被连接，将 **资产视图窗口** 内的视频文件拖拽至 **视频资产框** 内，设置视频播放 **帧率**、原始 **分辨率** 以及视频 **音量**，程序运行至此游戏会 **全屏播放视频**。

> [!NOTE]
>
> 在输入视频参数时，请务必确认 **参数设置与原视频的帧率和分辨率完全一致**，并且与画布尺寸保持正确的比例关系。**若参数不匹配，可能会导致画面拉伸变形，或出现大面积黑边。**

> [!CAUTION]
>
> 视频播放功能依赖 **FFmpeg**，请跟随该文档完成工具集成： [FFmpeg 集成教程](FFmpegInstallGuide.md)。
...
当该节点完成连接后，在 **场景文件** 输入框中填写 **application.scene.(自定义).lua**，即可将该自定义场景转换为基于脚本扩展的场景。该脚本需实现 **application.framework.scene.lua** 中定义的**on_enter、on_exit、on_update、on_render** 方法，以便在该场景内实现自定义的小游戏或小程序逻辑。

> [!NOTE]
>
> 当前版本中该功能尚不完善，请期待后续版本更详细的开发者文档。
...
> [!CAUTION]
>
>  **单文件导出** 依赖 **EnigmaVirtualBox**，请跟随该文档完成工具集成： [EnigmaVirtualBox 集成教程](EnigmaInstallGuide.md)。
