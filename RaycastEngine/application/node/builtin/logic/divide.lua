...
return Common.make_definition(NodeDef, function(ctx)
    ...
    builder:add_input({key = "left", type_id = "float", name = "被除数", default = 0})
    builder:add_input({key = "right", type_id = "float", name = "除数", default = 1})
    ...
    node.on_execute = function(self, scene)
        ...
        if right_val == 0 then
            NodeRuntimeHelper.abort(self, "除法运算错误，除数不能为 0")
        end

        local result = left_val / right_val
        if not NodeRuntimeHelper.is_finite_number(result) then
            NodeRuntimeHelper.abort(self, "除法运算结果不是有限数字")
        end
