...
local StyleDocument = require("application.framework.style_document")
...
local function _save_workspace_state()
    ...
    SettingsManager.set_workspace_style_state(persisted_open_guid_list, persisted_current_guid, {silent = true})
end
...
local function _resolve_style_guid(value)
    return ResourceIndex.resolve_guid("style", value)
end
...
function module.load_style_meta(meta)
    local document = StyleDocument.new(meta,
    {
        lazy_document = true,
        initial_open = false,
        manager = module,
...
function module.apply_workspace_state()
    local requested_open_guid_list = SettingsManager.get_open_style_guid_list and SettingsManager.get_open_style_guid_list() or {}
    local requested_current_guid = SettingsManager.get_current_style_guid and SettingsManager.get_current_style_guid() or ""
...
function module.load()
    module.begin_load()
    for _, meta in ipairs(ResourceIndex.list_by_type("style")) do
        module.load_style_meta(meta)
    end
...
function module.reconcile()
    ...
    for _, meta in ipairs(ResourceIndex.list_by_type("style")) do
        ...
        if document then
            ...
            table.insert(next_document_list, StyleDocument.new(meta,
            {
                lazy_document = true,
                initial_open = false,
                manager = module,
...
function module.find_by_guid(guid)
    local normalized_guid = _resolve_style_guid(guid)
...
function module.get_style_document(value)
...
function module.open_style_in_workspace(value, options)
    local document = _resolve_document(value)
...
function module.close_style_in_workspace(value)
    local document = _resolve_document(value)
    if not document then
        return false
    end
...
function module.set_workspace_current_style(value)
    local document = _resolve_document(value)
...
function module.resolve_parent_document(reference, requester, options)
    local resolve_options = _normalize_runtime_resolve_options(options)
    local guid = ResourceIndex.resolve_guid("style", reference)
    if not guid then
        return nil, "父样式不存在"
    end

    local document = document_by_guid[guid]
    if document and resolve_options.allow_unsaved_snapshot and _should_prefer_memory_snapshot(document) then
        local loaded_ok, err = _ensure_document_loaded(document)
        if not loaded_ok then
            return nil, err or "无法加载父样式"
        end
        return
        {
            guid = guid,
    ...
    local loaded_document, err = Style.load(meta.path)
...
function module.resolve_runtime_document(reference, options)
    ...
    return module.resolve_parent_document(reference, nil, resolve_options)
end
