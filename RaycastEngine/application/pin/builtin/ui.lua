...
return Common.make_definition({
    ...
    style_adapter = StyleAdapterFactory.make_resource_adapter("ui"),
    runtime =
    {
        validate = function(value, opts)
            local reference = ResourceIndex.make_reference("ui", value)
            if not reference then
                return false, nil,
                {
                    code = "resource_unset",
                    expected_type_id = "ui",
                    expected_display_name = "界面资源",
                }
            end

            if ResourceIndex.resolve_guid("ui", reference) == nil then
                return false, nil,
                {
    ...
    pin._apply_value = function(self, value)
        self._resource_ref = ResourceIndex.make_reference("ui", value)
    end

    pin._record_change = function(self, value)
        local old_ref = _clone_reference(self._resource_ref)
        local new_ref = ResourceIndex.make_reference("ui", value)
    ...
    pin._on_tick_widgets = function(self)
        imgui.BeginDisabled(not self._is_output and self._linked_pin_id)
            ...
            {
                popup_id = string.format("ui_pin_picker_%s", id),
                asset_type = "ui",
            ...
            if invalid_payload then
                LogManager.log(string.format("错误的引脚赋值类型，使用“%s”类型资源为“ui”类型引脚赋值", invalid_payload.type), "warning")
            elseif changed then
    ...
    pin.on_save = function(self)
        ...
        -- SAVE TRACE: concrete ui pin appends a cloned resource reference as val.
