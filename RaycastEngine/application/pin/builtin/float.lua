...
return Common.make_definition({
    ...
    style_adapter = StyleAdapterFactory.make_float_adapter(),
    runtime =
    {
    ...
    can_accept = function(input_pin, output_pin)
        if output_pin._type_id == "int" then
            return true
        end
    end
}, function(pin, ctx)
        ...
        pin._on_tick_widgets = function(self)
            imgui.BeginDisabled(not self._is_output and self._linked_pin_id)
                ...
                if imgui.IsItemDeactivatedAfterEdit() then
                    local next_value = pin._float.val
    ...
    pin.on_save = function(self)
        ...
        -- SAVE TRACE: concrete float pin appends its editable value.
    ...
    pin.get_val = function(self)
        return self:_resolve_runtime_input_value(self._float.val)
    end
end)
